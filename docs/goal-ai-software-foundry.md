...
- Active slice: [#145 - Arena mode production hardening](https://github.com/tonycdr-prog/architect-mcp/issues/145)
...
Current partial status: arena execution now requires file-plan review, explicit execution approval, at least two adapters, and isolated worktree mode. Candidate evidence records worktree, changed files, diff stat, adapter run issues, and review gates. `arena select <adapter>` moves one successful candidate into the normal promotion path without copying files immediately, while crashed, timed-out, cancelled, or otherwise failed candidates are blocked from normal selection. Fixture workflow tests cover isolated candidates, no auto-promotion, main-workspace non-mutation before selection, failed-candidate blocking, ranking evidence, selected-candidate promotion, disabled-isolation blocking, and release-gate readiness. This does not complete #145 by itself; final PR CI remains before a full `go`.
