...
- Active slice: [#146 - MCP catalog recommendation and install-plan flow in TUI](https://github.com/tonycdr-prog/architect-mcp/issues/146)
...
| 2026-05-17 | #146 MCP integrations slice: the TUI command palette now has `integrations recommend`, `integrations plan`, `integrations review`, dry-run `integrations apply`, `integrations approve`, and approval-gated `integrations write`. Fixture MCP tests prove generic database needs ask for provider clarification before Supabase can be planned, unknown servers fail closed, failed reviews block approval, dry-run apply does not write config, and `writeFiles=true` is only called after a separate TUI approval. Verification: `cargo test --workspace --test mcp_integrations`, `npm run docs:build`, `npm run release:check`, and green PR #158 CI. | `conditional go`: TUI install recommendations are now guarded by recommendation, review, and approval state; Copilot review remains before #146 is full `go`. |

## Current Slice Notes

The active implementation slice is [#146](https://github.com/tonycdr-prog/architect-mcp/issues/146). The desired outcome is a TUI integrations path where MCP server recommendations come from clarified need, install plans are dry-run and reviewed first, and project-local config writes require explicit human approval.

Current partial status: the TUI can recommend MCP servers from the live catalog, block plans for unrecommended servers, create and review dry-run install plans, run dry-run apply without file writes, and require `integrations approve <reason>` before `integrations write` can call the local-only apply tool with `writeFiles=true`. The clean release gate passes locally and PR #158 CI is green. This does not complete #146 by itself; Copilot review remains before a full `go`.
