...
Current partial status: `architect-mcp-tui governance-audit --json` now runs static governance checks and live `review_local_workspace mode=audit` drift evidence when MCP is available. It separates deterministic release gates from smoke evidence and returns only memory-safe durable proposals. A weekly/manual GitHub workflow produces public-safe step-summary evidence, and the governance audit issue template guides redacted public reports. This does not complete #148 by itself; broader read-only samples against maintained repos remain.
