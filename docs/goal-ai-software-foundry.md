...
Current partial status: `architect-mcp-tui governance-audit --json` now runs static governance checks and live `review_local_workspace mode=audit` drift evidence when MCP is available. It separates deterministic release gates from smoke evidence and returns only memory-safe durable proposals. This does not complete #148 by itself; recurring QA workflows, public-safe issue evidence, and broader repo samples remain.
