...
Current partial status: structured prompts and interactive answers can now produce the typed brief shape required by live `grill_me`, generated plan checks and repo-layout file hints are included in the pre-edit contract, adapter execution now needs a separate approval from promotion, approved adapter prompts include the contract and verification rules, the default Codex adapter uses non-interactive `codex exec`, final/session review now requires passed verification evidence, final review refreshes implementation drift review with that evidence, operators can list the exact required verification checks before recording results, promotion readiness reports blockers and next actions before copying files, failed adapter runs are durable promotion blockers unless explicitly overridden or rerun successfully after fresh approval, a real local Codex promotion smoke now reaches isolated worktree diff evidence, verification, final/session review, separate promotion approval, and single-file promotion, the scripted command-palette walkthrough exercises the full happy path in a throwaway workspace, source-built terminal smoke passes through both the direct binary and npm shim paths with adapter-availability warnings only, and local macOS manual launch now keeps all primary panels visible at 80x24. This does not complete #143 by itself; remaining work includes broader manual terminal rendering QA on Linux and Windows.
