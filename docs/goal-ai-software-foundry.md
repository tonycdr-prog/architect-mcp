...
Current partial status: the TUI can recommend MCP servers from the live catalog, block plans for unrecommended servers, create and review dry-run install plans, run dry-run apply without file writes, and require `integrations approve <reason>` before `integrations write` can call the local-only apply tool with `writeFiles=true`. The session persists metadata summaries instead of raw MCP config payloads, and stale integration evidence is cleared after changed answers. The clean release gate passes locally. This does not complete #146 by itself; final PR CI and Copilot review disposition remain before a full `go`.
