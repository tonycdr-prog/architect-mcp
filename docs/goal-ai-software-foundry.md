...
Current partial status: the TUI can create a private-by-default repo foundry plan after file-plan review, show scaffold artifact requirements, require `foundry approve <reason>` before `foundry create`, and keep `foundry create` as a dry-run preview that performs no GitHub mutation. This does not complete #147 by itself; live private repo creation, initial commit generation, first PR creation, and smoke evidence remain before a full `go`.
