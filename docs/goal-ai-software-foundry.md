...
- Active slice: [#148 - Governance audit and drift evidence loop](https://github.com/tonycdr-prog/architect-mcp/issues/148)
...
The active implementation slice is [#148](https://github.com/tonycdr-prog/architect-mcp/issues/148). The desired outcome is a read-only governance and drift loop where maintained repositories can be audited for stale docs, weak tests, unsafe config, contract drift, release-gate health, and memory-policy safety without code mutation.

Current partial status: `architect-mcp-tui governance-audit --json` now runs static governance checks and live `review_local_workspace mode=audit` drift evidence when MCP is available. It separates deterministic release gates from smoke evidence and returns only repo-scoped memory-safe durable proposals. A weekly/manual GitHub workflow produces public-safe step-summary evidence, and the governance audit issue template guides redacted public reports. Broader public-repo read-only samples have been run successfully; the remaining #148 work is final gate/CI evidence and stacked PR landing.
