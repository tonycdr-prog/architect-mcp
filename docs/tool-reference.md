...
The default MCP surface exposes the core work-gate tools. Set `ARCHITECT_MCP_TOOL_SURFACE=advanced` to expose the full standards, stack-pack, governance, repo-quality, productization boundary, and eval surface.

## Core Surface

- `grill_me`: pressure-test a brief, infer stack packs, and optionally produce contract artifacts.
- `create_pre_edit_contract`: capture intent, likely files, non-goals, assumptions, and verification checks before edits.
- `review_build_plan`: check ordered implementation slices and exact verification commands.
- `review_proposed_file_plan`: review proposed files before writes happen.
- `review_repo_structure`: review supplied file summaries and directories.
- `review_implementation_against_contract`: detect implementation drift and skipped verification.
- `review_agent_final_response`: review changed, verified, assumptions, not-done, and evidence honesty.
- `review_agent_session`: combine intent, contract, changed files, verification, memory, and final response quality.

## Project Intake And Artifacts
...
## Stack Packs And Source Guidance
...
## Governance, Operating Model, And Evals
...
Hosted-safe tools can run without reading or writing the server filesystem. These local-only tools are excluded from hosted HTTP mode:

- `promote_stack_pack_to_files`
- `review_local_workspace`
- `scan_mcp_config_files`

Memory tools are classified as future adapters because persistence must be an explicit client or product decision.
