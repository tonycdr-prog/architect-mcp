...
pub(crate) fn help_update() -> WorkflowUpdate {
    update(
        vec![
            "commands: new app <idea>, resume <session-id>, answer key=value, grill, contract, review plan, review files, run adapter, diff summary, diff file <path>, approve [reason], reject [reason], override [reason], promotion status, promote, arena run <adapter[,adapter]>, arena rank, verification status, record verification <required check>=passed, final review <text>, session review".to_string(),
        ],
        Vec::new(),
        None,
    )
