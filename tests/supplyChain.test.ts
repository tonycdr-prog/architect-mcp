...
describe("supply-chain and release hygiene", () => {
  ...
  it("keeps the evolved goal ledger aligned with the active slice and launch boundary", () => {
    ...
    assert.match(goal, /Active slice: \[#328 - Document Foundry Mode operating contract and roadmap\]/);
    assert.match(goal, /comprehensive, PR-specific `@copilot` review/);
    assert.match(goal, /#327 eval-corpus slice landed in \[PR #338\]/);
    assert.match(goal, /#328.*Foundry operating-contract docs slice/);
    assert.match(goal, /Current Shipped Behavior from Future Planned Behavior/);
    assert.match(goal, /approval-required preview footer/);
    ...
    assert.match(goal, /Linux VM terminal evidence for #136/);
    assert.match(goal, /explicit maintainer waiver for the remaining Windows terminal-evidence gap/);
    assert.match(goal, /This is a waiver, not Windows proof/);
    ...
    assert.match(goal, /#284 in PR #307/);
    assert.match(goal, /#150 as the first merge candidate and #307 as the current stack head/);
    assert.doesNotMatch(goal, /unresolved review threads on PRs #235, #240, and #243/);
    assert.match(goal, /Current launch readiness from stack head #307 is `go` only when the explicit #136 blocker and terminal-evidence waivers are supplied/);
    assert.match(goal, /without the waiver, #136 still visibly reports missing Windows terminal proof/);
  ...
  it("documents Foundry Mode without claiming unapproved mutation", () => {
    const foundry = readFileSync("docs/foundry-mode.md", "utf8");
    const readme = readFileSync("README.md", "utf8");
    const docsHome = readFileSync("docs/index.md", "utf8");
    const vitepress = readFileSync("docs/.vitepress/config.mts", "utf8");
    const llms = readFileSync("llms.txt", "utf8");

    assert.match(foundry, /Current Shipped Behavior/);
    assert.match(foundry, /Future Planned Behavior/);
    assert.match(foundry, /No autonomous external mutation/);
    assert.match(foundry, /approval-required preview/);
    assert.match(foundry, /Generated by architect-mcp Foundry Mode as an approval-required preview: https:\/\/github\.com\/tonycdr-prog\/architect-mcp/);
    assert.match(foundry, /Repository PR templates are hard maintainer signals/);
    assert.match(foundry, /Recent accepted PR headings are advisory fallback/);
    for (const issue of [310, 311, 312, 313, 314, 315, 316, 317, 318, 319]) {
      assert.match(foundry, new RegExp(`\\[#${issue}\\]\\(https://github\\.com/tonycdr-prog/architect-mcp/issues/${issue}\\)`));
    }
    assert.doesNotMatch(foundry, /automatically creates? (branches|commits|issues|pull requests|comments|labels)/i);
    assert.match(readme, /foundry-mode/);
    assert.match(docsHome, /Foundry Mode/);
    assert.match(vitepress, /Foundry Mode/);
    assert.match(llms, /docs\/foundry-mode\.md/);
  });
